...
public class CronExpression implements Serializable, Cloneable {
	...
	protected transient com.taobao.pamirs.schedule.MonLdaySet monLdaySet;
	...
	public static boolean isValidExpression(String cronExpression) {
		...
		try {
			new com.taobao.pamirs.schedule.CronExpression(cronExpression);
	...
	protected void buildExpression(String expression) throws ParseException {
		...
		try {
			...
			while (exprsTok.hasMoreTokens() && exprOn <= YEAR) {
				...
				if (exprOn == DAY_OF_WEEK && expr.indexOf('L') != -1
						&& expr.length() > 1 && expr.indexOf(",") >= 0) {
					throw new ParseException(
	...
	protected int storeExpressionVals(int pos, String s, int type)
		...
		if (c == '*' || c == '/') {
			...
			if (i >= s.length()) {
				...
				c = s.charAt(i);
				if (c >= '0' && c <= '9') {
					com.taobao.pamirs.schedule.ValueSet vs = getValue(val, s, i);
					val = vs.value;
					i = vs.pos;
				}
	...
	protected int checkNext(int pos, String s, int val, int type)
		...
		if (c == 'L') {
			if (type == DAY_OF_WEEK) {
				...
				//compatible for expression like '2L'
				if (monLdaySet == null) {
					monLdaySet = new com.taobao.pamirs.schedule.MonLdaySet();
				}
				lastdayOfMonth = true;
				monLdaySet.isLast = true;
				monLdaySet.value = 0;
				monLdaySet.ex = val;
				i++;
				if (i < s.length()) {
					throw new ParseException("invalid expression or char", i);
				}
				return i;
			} else {
		...
		if (c == '-') {
			i++;
			c = s.charAt(i);
			//compatible for expression like '5-L'
			if (c == 'L') {
				if (monLdaySet == null) {
					monLdaySet = new com.taobao.pamirs.schedule.MonLdaySet();
				}
				monLdaySet.isLast = true;
				monLdaySet.value = val;
				monLdaySet.ex = 1;
				i++;
				if (i < s.length()) {
					throw new ParseException("invalid expression or char", i);
			...
			c = s.charAt(i);
			if (c >= '0' && c <= '9') {
				com.taobao.pamirs.schedule.ValueSet vs = getValue(v, s, i);
				int v1 = vs.value;
				end = v1;
				i = vs.pos;
				//compatible for expression like '5-10L'
				if (i < s.length()) {
					c = s.charAt(i);
					if (c== 'L') {
						if (monLdaySet == null) {
			...
			//compatible for expression like '5-2L'
			if (c == 'L') {
				if (monLdaySet == null) {
					monLdaySet = new com.taobao.pamirs.schedule.MonLdaySet();
				}
			...
			if (i < s.length() && ((c = s.charAt(i)) == '/')) {
				...
				c = s.charAt(i);
				if (c >= '0' && c <= '9') {
					com.taobao.pamirs.schedule.ValueSet vs = getValue(v2, s, i);
					int v3 = vs.value;
	...
	protected String getExpressionSetSummary(java.util.Set set) {
		...
		if (!itr.hasNext() && lastdayOfMonth) {
			buf
					.append("you set last day of the month, the day will be got until a defined time we get");
		}
		if (!itr.hasNext() && monLdaySet != null && monLdaySet.isLast) {
			buf
					.append("'L' is in your month setting area, the day will be got until a defined time we get");
		}
		boolean first = true;
		while (itr.hasNext()) {
	...
	protected void addToSet(int val, int end, int incr, int type)
		...
		if (type == SECOND || type == MINUTE) {
			...
			if (stopAt == -1) {
				stopAt = CronTrigger_YEAR_TO_GIVEUP_SCHEDULING_AT;
			}
			if (startAt == -1 || startAt == ALL_SPEC_INT) {
				startAt = 1970;
			}
	...
	protected com.taobao.pamirs.schedule.ValueSet getValue(int v, String s, int i) {
		char c = s.charAt(i);
		...
		com.taobao.pamirs.schedule.ValueSet val = new com.taobao.pamirs.schedule.ValueSet();
	...
	protected Date getTimeAfter(Date afterTime) {
		...
		while (!gotOne) {
			...
			if (dayOfMSpec && !dayOfWSpec) { // get day by day of month rule
				int endDays = 0;
				if (monLdaySet != null && monLdaySet.isLast && !endDaysFlag) {
					int end = getLastDayOfMonth(mon, cl.get(Calendar.YEAR))
							- monLdaySet.ex + 1;
					if (monLdaySet.value == 0) {
						endDays = end;
					} else {
						try {
							if (monLdaySet.value > end) {
								throw new IllegalArgumentException(
										"Start day must be less than end day");
							}
							addToSet(monLdaySet.value, end, 1, DAY_OF_MONTH);
				...
				if (lastdayOfMonth && !endDaysFlag) {
					if (!nearestWeekday) {
						if (monLdaySet != null && monLdaySet.isLast) {
							try {
								getSet(DAY_OF_MONTH).clear();
								addToSet(endDays, -1, -1, DAY_OF_MONTH);
								endDaysFlag = true;
								continue;
							} catch (ParseException e) {
								e.printStackTrace();
			...
			// but keep looping...
			if (year > CronTrigger_YEAR_TO_GIVEUP_SCHEDULING_AT) {
				return null;
			}
	...
	public Object clone() {
		com.taobao.pamirs.schedule.CronExpression copy = null;
		try {
			copy = new com.taobao.pamirs.schedule.CronExpression(getCronExpression());
			if (getTimeZone() != null)

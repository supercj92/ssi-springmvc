...
import com.taobao.pamirs.schedule.AbstractDynamicMBean;
...
public class TBScheduleManagerFactoryMBean extends AbstractDynamicMBean {
	...
	public String createTaskType(String baseTaskType,String dealBeanName,String queueIds) {
		ScheduleTaskType taskType = new ScheduleTaskType();
		taskType.setBaseTaskType(baseTaskType);
		taskType.setDealBeanName(dealBeanName);
		try {
			tbScheduleManagerFactory.getScheduleConfigCenter().createBaseTaskType(taskType,queueIds.split(","));
			return "创建成功";
		} catch (Exception e) {
			return e.toString();
		}
	...
	public String[] getTaskTypeList() throws Exception{
		List<ScheduleTaskType> taskTypes = tbScheduleManagerFactory.getScheduleConfigCenter().getAllTaskTypeBaseInfo();
	...
	protected void buildDynamicMBeanInfo() {
		...
		MBeanOperationInfo[] dOperations = new MBeanOperationInfo[] {
				...
				new MBeanOperationInfo("createTaskType", "创建调度服务",
						new MBeanParameterInfo[] {
								...
								new MBeanParameterInfo("queueIds",
										"java.lang.String",
										"任务队列ID,以逗号分隔")},
						"String", MBeanOperationInfo.ACTION),
		...
		dMBeanInfo = new MBeanInfo(this.getClass().getName(), "QueueDefine", dAttributes, null, dOperations, null);
	}
}

...
public class TBScheduleProcessorNotSleep<T> implements IScheduleProcessor, Runnable {
	...
	private static transient Log logger = LogFactory.getLog(com.taobao.pamirs.schedule.TBScheduleProcessorNotSleep.class);
	...
	public TBScheduleProcessorNotSleep(TBScheduleManager aManager,
		...
		this.taskComparator = new MYComparator(this.taskDealProcessor.getComparator());
	...
	protected int loadScheduleData() {
		...
		try {
			...
			try {
				List<String> taskQueues = this.scheduleManager
						.getCurrentScheduleQueue();
				// 根据队列信息查询需要调度的数据，然后增加到任务列表中
				if (taskQueues.size() > 0) {
					List<T> tmpList = this.taskDealProcessor.selectTasks(
							scheduleManager.getTaskTypeRunningInfo()
									.getOwnSign(), this.taskTypeInfo
									.getTaskQueueNumber(), taskQueues,
